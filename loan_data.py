...
ECOS_KEY = os.getenv("ECOS_KEY")    # GitHub Actions에서 환경 변수로 넣을 예정


WORK_DIR = Path(__file__).parent
OUT_DIR = WORK_DIR / 'output'
DATA = OUT_DIR / 'loan_data.xlsx'

def excel_data():
    # 데이터 코드 예시
    INDICATORS = [["일반신용대출", "121Y006", "M", "BECBLA03051", 100],
                    ["주택담보대출", "121Y006", "M", "BECBLA0302", 100]]
    with pd.ExcelWriter(DATA) as writer:
        ecos = Ecos(ECOS_KEY)
        for name, state_code, freq, item_code1, limit in INDICATORS:
